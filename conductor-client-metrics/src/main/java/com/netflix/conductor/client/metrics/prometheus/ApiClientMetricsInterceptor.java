 ...
 *
 * <p>Wire it in via the existing {@code configureOkHttp} hook on
 * {@code ConductorClient.Builder}:
 * <pre>{@code
 * PrometheusApiClientMetrics metrics = new PrometheusApiClientMetrics(
 *         prometheusMetricsCollector.getRegistry());
 *
 * ApiClient client = ApiClient.builder()
 *         .basePath("https://conductor.example/api")
 *         .credentials(keyId, secret)
 *         .configureOkHttp(b -> b.addInterceptor(
 *                 new ApiClientMetricsInterceptor(metrics)))
 *         .build();
 * }</pre>
 *
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        try {
            ...
            try {
                ...
                // Use the URL path (interpolated) for the uri label. The
                // OkHttp Request object has already had path parameters
                // substituted in at this point, which matches the python-sdk
                // behavior. Operators who care about cardinality can
                ...
                // Never let metric bookkeeping affect the HTTP call.
            }
        }
    }
}
