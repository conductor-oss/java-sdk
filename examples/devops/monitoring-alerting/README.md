...
Orchestrates a monitoring and alerting pipeline using [Conductor](https://github.com/conductor-oss/conductor). Incoming alerts are evaluated for severity, deduplicated against recent history, enriched with deployment context, and routed to the appropriate on-call channel. You write the alert-processing logic, Conductor handles retries, failure routing, durability, and observability for free.
...
```
=== Example monitoring-alerting: Monitoring and Alerting ===

Step 1: Registering task definitions...
  Registered: ma_evaluate, ma_deduplicate, ma_enrich, ma_route

Step 2: Registering workflow 'monitoring_alerting_workflow'...
  Workflow registered.
...
Step 4: Starting workflow...
  ...
  [dedup] 1 unique from 5 raw alerts
  [enrich] Added deployment context
  [evaluate]
  [route] Sent to on-call via Slack
...
conductor workflow start \
  ...
  --version 1 \
  --input '{"alertName": "sample-name", "high-cpu": "sample-high-cpu", "metric": "sample-metric", "cpu": "sample-cpu", "value": "sample-value"}'
```
