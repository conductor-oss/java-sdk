...
Orchestrates a container build-scan-deploy pipeline using [Conductor](https://github.com/conductor-oss/conductor). This workflow builds a container image, scans it for vulnerabilities, deploys it to a Kubernetes cluster, and runs post-deployment health checks. You write the container logic, Conductor handles retries, failure routing, durability, and observability for free.

## Shipping Containers Safely
...
```
=== Example 333: Container Orchestratio ===

Step 1: Registering task definitions...
  Registered: co_build, co_scan, co_deploy, co_health_check

Step 2: Registering workflow 'container_orchestration_workflow'...
  Workflow registered.
...
Step 4: Starting workflow...
  ...
  [build] Built user-service:v2.1.0
  [deploy] Deployed to cluster prod-east
  [health] All containers healthy
  [scan] No critical vulnerabilities found
...
conductor workflow start \
  ...
  --version 1 \
  --input '{"image": "sample-image", "user-service": "sample-user-service", "tag": "sample-tag", "v2.1.0": "sample-v2.1.0", "cluster": "sample-cluster"}'
```
