...
Orchestrates safe DNS record changes using [Conductor](https://github.com/conductor-oss/conductor). This workflow plans a DNS change, validates it against existing records for conflicts, applies it to the DNS provider, and verifies propagation. You write the DNS logic, Conductor handles retries, failure routing, durability, and observability for free.
...
```
=== Example 345: DNS Management ===

Step 1: Registering task definitions...
  Registered: dns_plan, dns_validate, dns_apply, dns_verify

Step 2: Registering workflow 'dns_management_workflow'...
  Workflow registered.
...
Step 4: Starting workflow...
  ...
  [apply] DNS records updated in Route53
  [plan] DNS change planned
  [validate] No conflicts detected
  [verify] DNS propagation confirmed
...
conductor workflow start \
  ...
  --version 1 \
  --input '{"domain": "sample-domain", "api.example.com": "sample-api.example.com", "recordType": "standard", "CNAME": "sample-name", "target": "sample-target"}'
```
