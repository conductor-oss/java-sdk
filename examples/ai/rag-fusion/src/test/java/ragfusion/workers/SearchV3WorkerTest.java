...
class SearchV3WorkerTest {

    private final SearchV3Worker worker = new SearchV3Worker();

    @Test
    void taskDefName() {
        assertEquals("rf_search_v3", worker.getTaskDefName());
    }

    @Test
    void returnsFourResults() {
        Task task = taskWith(new HashMap<>(Map.of("query", "test query", "variantIndex", 3)));
    ...
    void returnsSourceIdentifier() {
        ...
        assertEquals("search_v3", result.getOutputData().get("source"));
    }

    @Test
    void idsHaveV3Prefix() {
        Task task = taskWith(new HashMap<>(Map.of("query", "test")));
        TaskResult result = worker.execute(task);
        ...
        for (Map<String, Object> doc : results) {
            assertTrue(((String) doc.get("id")).startsWith("v3-"));
        }
    }
