...
class SearchV1WorkerTest {

    private final SearchV1Worker worker = new SearchV1Worker();

    @Test
    void taskDefName() {
        assertEquals("rf_search_v1", worker.getTaskDefName());
    }

    @Test
    void returnsFourResults() {
        Task task = taskWith(new HashMap<>(Map.of("query", "test query", "variantIndex", 1)));
    ...
    void returnsSourceIdentifier() {
        ...
        assertEquals("search_v1", result.getOutputData().get("source"));
    }

    @Test
    void idsHaveV1Prefix() {
        Task task = taskWith(new HashMap<>(Map.of("query", "test")));
        TaskResult result = worker.execute(task);
        ...
        for (Map<String, Object> doc : results) {
            assertTrue(((String) doc.get("id")).startsWith("v1-"));
        }
    }
