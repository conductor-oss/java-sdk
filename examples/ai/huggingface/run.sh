...
echo "=== Huggingface (Java) ==="
echo ""

if [ -n "$HUGGINGFACE_TOKEN" ]; then
    echo "Mode: LIVE (HUGGINGFACE_TOKEN is set)"
else
    echo "Mode: SIMULATED (set HUGGINGFACE_TOKEN for live API calls)"
fi
echo ""
...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" java -jar target/huggingface-1.0.0.jar "$@"
else
