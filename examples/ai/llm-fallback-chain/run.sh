...
echo "=== Llm Fallback Chain (Java) ==="
echo ""

MISSING=""
[ -z "$CONDUCTOR_OPENAI_API_KEY" ] && MISSING="$MISSING CONDUCTOR_OPENAI_API_KEY"
[ -z "$CONDUCTOR_ANTHROPIC_API_KEY" ] && MISSING="$MISSING CONDUCTOR_ANTHROPIC_API_KEY"
[ -z "$GOOGLE_API_KEY" ] && MISSING="$MISSING GOOGLE_API_KEY"
if [ -n "$MISSING" ]; then
  echo "ERROR: Required API keys are missing:$MISSING"
  echo "  Set them in your .env file or environment."
  exit 1
fi
echo "API keys detected: OpenAI, Anthropic, Google AI"
echo ""
...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" java -jar target/llm-fallback-chain-1.0.0.jar "$@"
else
