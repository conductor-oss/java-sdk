...
echo "=== Rag Multi Query (Java) ==="
echo ""

# Mode detection
if [ -n "$CONDUCTOR_OPENAI_API_KEY" ]; then
  echo "Mode: LIVE (CONDUCTOR_OPENAI_API_KEY detected)"
else
  echo "Mode: SIMULATED (set CONDUCTOR_OPENAI_API_KEY for real API calls)"
fi
echo ""
...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" CONDUCTOR_OPENAI_API_KEY="${CONDUCTOR_OPENAI_API_KEY:-}" java -jar target/rag-multi-query-1.0.0.jar "$@"
else
