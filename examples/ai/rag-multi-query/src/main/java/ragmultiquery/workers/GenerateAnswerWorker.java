...
/**
 * Worker that generates a final answer from deduplicated context documents.
...
public class GenerateAnswerWorker implements Worker {
    ...
    public TaskResult execute(Task task) {
        ...
        if (openaiApiKey != null && !openaiApiKey.isBlank()) {
            try {
                ...
                System.out.println("  [llm] Answer (live OpenAI) from " + contextSize + " unique docs");
            ...
            System.out.println("  [llm] Generating answer from " + contextSize + " unique docs...");

            String answer = "Workflow orchestration with Conductor provides centralized control, "
                    + "automatic retries and timeouts, versioned deployments, and decoupled task "
                    + "execution. Unlike choreography, it avoids hidden dependencies.";

            result.setStatus(TaskResult.Status.COMPLETED);
            result.getOutputData().put("answer", answer);
            result.getOutputData().put("tokensUsed", 134);
        }
    ...
    private String callOpenAIChat(String question, String contextText, TaskResult result) throws Exception {
        String systemPrompt = "You are a helpful assistant. Answer the user's question based on the provided context "
                + "from multi-query retrieval (multiple search perspectives, deduplicated).";
