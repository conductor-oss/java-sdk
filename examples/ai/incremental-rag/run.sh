...
if [ -n "$CONDUCTOR_OPENAI_API_KEY" ]; then
    ...
    echo "  - Vector store workers remain simulated (no vector DB)"
else
...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" java -jar target/incremental-rag-1.0.0.jar "$@"
else
