...
echo "=== Conversational Rag (Java) ==="
echo ""

if [ -n "$CONDUCTOR_OPENAI_API_KEY" ]; then
    echo "Mode: LIVE (OpenAI API key detected)"
    echo "  - Embed worker will call OpenAI Embeddings API (text-embedding-3-small)"
    echo "  - Generate worker will call OpenAI Chat Completions (gpt-4o-mini)"
    echo "  - Search/retrieve workers remain simulated (no vector DB)"
else
    echo "Mode: SIMULATED (no CONDUCTOR_OPENAI_API_KEY set)"
    echo "  Tip: export CONDUCTOR_OPENAI_API_KEY=sk-... to enable live OpenAI calls"
fi
echo ""
...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" java -jar target/conversational-rag-1.0.0.jar "$@"
else
