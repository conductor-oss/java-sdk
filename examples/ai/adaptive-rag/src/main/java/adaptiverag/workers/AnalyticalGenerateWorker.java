...
/**
 * Analytical generation worker that synthesizes an answer from multi-hop reasoning.
 *
 * Requires CONDUCTOR_OPENAI_API_KEY to be set. Calls OpenAI Chat Completions API (gpt-4o-mini).
 */
public class AnalyticalGenerateWorker implements Worker {

    private final String openaiApiKey;
    private final HttpClient httpClient;
    private final ObjectMapper objectMapper;

    public AnalyticalGenerateWorker() {
    ...
    public TaskResult execute(Task task) {
        ...
        try {
            ...
            result.getOutputData().put("strategy", "multi_hop_rag");
        } catch (Exception e) {
            System.err.println("  [analytical-gen] OpenAI call failed: " + e.getMessage());
