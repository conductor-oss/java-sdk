...
echo "  - Classify, generate, and reasoning workers will call OpenAI (gpt-4o-mini)"
echo "  - Search/retrieve workers use Jaccard similarity over bundled docs"
echo ""
...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" java -jar target/adaptive-rag-1.0.0.jar "$@"
else
