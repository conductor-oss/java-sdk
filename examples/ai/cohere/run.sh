...
if [ -n "$COHERE_API_KEY" ]; then
    echo "Mode: LIVE (COHERE_API_KEY is set)"
else
    echo "Mode: SIMULATED (set COHERE_API_KEY for live API calls)"
fi
echo ""
...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" java -jar target/cohere-1.0.0.jar "$@"
else
