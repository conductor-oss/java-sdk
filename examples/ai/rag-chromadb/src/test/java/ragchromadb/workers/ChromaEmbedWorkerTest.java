...
class ChromaEmbedWorkerTest {

    private final ChromaEmbedWorker worker = new ChromaEmbedWorker();

    @Test
    void taskDefName() {
        assertEquals("chroma_embed", worker.getTaskDefName());
    }

    @Test
    void returnsFixedEmbedding() {
        Task task = taskWith(new HashMap<>(Map.of("question", "How does ChromaDB store embeddings?")));
