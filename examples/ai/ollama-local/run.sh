...
if curl -sf "$OLLAMA_URL/api/tags" > /dev/null 2>&1; then
    echo "Mode: LIVE (Ollama detected at $OLLAMA_URL)"
else
    echo "Mode: SIMULATED (Ollama not available at $OLLAMA_URL. Start Ollama for live inference.)"
fi
echo ""
...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" java -jar target/ollama-local-1.0.0.jar "$@"
else
