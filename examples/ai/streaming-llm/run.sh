...
echo "=== Streaming Llm (Java) ==="
echo ""

if [ -n "$CONDUCTOR_OPENAI_API_KEY" ]; then
    echo "Mode: LIVE (CONDUCTOR_OPENAI_API_KEY is set)"
else
    echo "Mode: SIMULATED (set CONDUCTOR_OPENAI_API_KEY for live streaming)"
fi
echo ""
...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" java -jar target/streaming-llm-1.0.0.jar "$@"
else
