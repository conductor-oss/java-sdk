...
class QdrantEmbedWorkerTest {

    private final QdrantEmbedWorker worker = new QdrantEmbedWorker();

    @Test
    void taskDefName() {
        assertEquals("qdrant_embed", worker.getTaskDefName());
    }

    @Test
    void returnsFixedEmbedding() {
        Task task = taskWith(new HashMap<>(Map.of("question", "How does Qdrant filter during search?")));
        ...
        assertEquals(0.1234, vec.get(0));
        assertEquals(-0.0123, vec.get(7));
    }
    ...
    void embeddingIsDeterministic() {
        Task task1 = taskWith(new HashMap<>(Map.of("question", "first")));
        Task task2 = taskWith(new HashMap<>(Map.of("question", "second")));
