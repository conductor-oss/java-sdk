...
echo "  - Generate workers will call OpenAI (gpt-4o-mini)"
echo "  - Web search worker fetches real results from Wikipedia"
echo ""
...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" java -jar target/corrective-rag-1.0.0.jar "$@"
else
