...
echo "=== Mistral Ai (Java) ==="
echo ""

if [ -n "$MISTRAL_API_KEY" ]; then
    echo "Mode: LIVE (MISTRAL_API_KEY is set)"
else
    echo "Mode: SIMULATED (set MISTRAL_API_KEY for live API calls)"
fi
echo ""
...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" java -jar target/mistral-ai-1.0.0.jar "$@"
else
