...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" OPENAI_CHAT_MODEL="$OPENAI_CHAT_MODEL" OPENAI_EMBED_MODEL="$OPENAI_EMBED_MODEL" java -jar target/basic-rag-1.0.0.jar "$@"
else
