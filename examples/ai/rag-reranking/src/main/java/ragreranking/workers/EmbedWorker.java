 ...
 * Otherwise falls back to a fixed deterministic.embedding.
 */
public class EmbedWorker implements Worker {
    ...
    public TaskResult execute(Task task) {
        ...
        if (openaiApiKey != null && !openaiApiKey.isBlank()) {
            try {
                List<Double> embedding = callOpenAIEmbedding(question, result);
                if (embedding == null) {
                    return result;
                }
                System.out.println("  [embed] Query (live OpenAI): \"" + question + "\" -> " + embedding.size() + " dims");
                result.setStatus(TaskResult.Status.COMPLETED);
                result.getOutputData().put("embedding", embedding);
            } catch (Exception e) {
                System.err.println("  [embed] OpenAI API error: " + e.getMessage());
