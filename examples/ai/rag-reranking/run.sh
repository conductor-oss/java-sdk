...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" CONDUCTOR_OPENAI_API_KEY="${CONDUCTOR_OPENAI_API_KEY:-}" java -jar target/rag-reranking-1.0.0.jar "$@"
else
