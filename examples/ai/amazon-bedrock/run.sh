...
echo "=== Amazon Bedrock (Java) ==="
echo ""

if [ -n "$AWS_ACCESS_KEY_ID" ] && [ -n "$AWS_SECRET_ACCESS_KEY" ]; then
    echo "Mode: SIMULATED (AWS credentials detected, but live Bedrock requires AWS SDK v2)"
else
    echo "Mode: SIMULATED (set AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY for production use)"
fi
echo ""
...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" java -jar target/amazon-bedrock-1.0.0.jar "$@"
else
