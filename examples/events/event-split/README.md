...
Each sub-event processor is a simple, independent worker. a plain Java class that does one thing. Conductor takes care of splitting the composite event, processing all sub-events in parallel via FORK_JOIN, aggregating results after all complete, retrying any failed sub-event processor independently, and tracking the entire split-and-process operation. You get all of that, without writing a single line of orchestration code.
