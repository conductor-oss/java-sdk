...
Event fan-out workflow that receives an event, fans out to analytics, storage, and notification processing in parallel via FORK_JOIN, then aggregates the results. Uses [Conductor](https://github.com/conductor-oss/conductor) to orchestrate independent services as workers .  you write the business logic, Conductor handles retries, failure routing, durability, and observability for free.
...
Step 1: Registering task definitions...
  Registered: fo_receive_event, fo_analytics, fo_storage, fo_notification, fo_aggregate

Step 2: Registering workflow 'event_fanout_wf'...
  Workflow registered.
...
Step 4: Starting workflow...
  ...
  [fo_aggregate] Aggregating results: analytics=
  [fo_analytics] Tracking analytics for event:
  [fo_notification] Notifying via slack for event:
  [fo_receive_event] Received event:
  [fo_storage] Storing event

  Status: COMPLETED
  Output: {processorResults=..., result=..., metricsUpdated=..., channel=...}
...
conductor workflow start \
  ...
  --version 1 \
  --input '{"eventId": "evt-fanout-001", "evt-fanout-001": "eventType", "eventType": "order.created", "order.created": "payload", "payload": {"key": "value"}}'
```
