...
Event-driven workflow that receives events, classifies them by type, and routes to the appropriate handler via a SWITCH task. Uses [Conductor](https://github.com/conductor-oss/conductor) to orchestrate independent services as workers .  you write the business logic, Conductor handles retries, failure routing, durability, and observability for free.
...
Step 1: Registering task definitions...
  Registered: ed_receive_event, ed_classify_event, ed_handle_order, ed_handle_payment, ed_handle_generic

Step 2: Registering workflow 'event_driven_wf'...
  Workflow registered.
...
Step 4: Starting workflow...
  ...
  [ed_classify_event] Classifying event type:
  [ed_handle_generic] Processing generic event (category:
  [ed_handle_order] Processing order
  [ed_handle_payment] Processing payment
  [ed_receive_event] Received event type:

  Status: COMPLETED
  Output: {category=..., priority=..., handler=..., processed=...}
...
conductor workflow start \
  ...
  --version 1 \
  --input '{"eventId": "evt-fixed-001", "evt-fixed-001": "eventType", "eventType": "order.created", "order.created": "eventData", "eventData": {"key": "value"}}'
```
