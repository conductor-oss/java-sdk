...
class ProcessBatchWorkerTest {
    ...
    @Test
    void taskDefName() {
        assertEquals("pr_process_batch", worker.getTaskDefName());
    }

    @Test
    void processesEventWithPayload() {
        Task task = taskWith(Map.of("eventId", "evt-001", "payload", "inventory.low"));
        TaskResult result = worker.execute(task);

        assertEquals(TaskResult.Status.COMPLETED, result.getStatus());
        assertEquals(true, result.getOutputData().get("processed"));
        assertEquals("batch", result.getOutputData().get("lane"));
    }
    ...
    void handlesNullEventId() {
        ...
        input.put("payload", "inventory.low");
    ...
    void processesMultipleEventsConsistently() {
        ...
        assertEquals("batch", result1.getOutputData().get("lane"));
        assertEquals("batch", result2.getOutputData().get("lane"));
