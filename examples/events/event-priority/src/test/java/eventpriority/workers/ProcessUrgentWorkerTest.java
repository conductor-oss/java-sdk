...
class ProcessUrgentWorkerTest {

    private final ProcessUrgentWorker worker = new ProcessUrgentWorker();

    @Test
    void taskDefName() {
        assertEquals("pr_process_urgent", worker.getTaskDefName());
    }

    @Test
    void processesEventWithPayload() {
        Task task = taskWith(Map.of("eventId", "evt-001", "payload", "payment.failed"));
        TaskResult result = worker.execute(task);

        assertEquals(TaskResult.Status.COMPLETED, result.getStatus());
        assertEquals(true, result.getOutputData().get("processed"));
        assertEquals("urgent", result.getOutputData().get("lane"));
    }
    ...
    @Test
    void outputAlwaysHasLaneUrgent() {
    ...
    void handlesNullEventId() {
        ...
        input.put("payload", "payment.failed");
    ...
    void processesMultipleEventsConsistently() {
        ...
        assertEquals("urgent", result1.getOutputData().get("lane"));
        assertEquals("urgent", result2.getOutputData().get("lane"));
