...
Event merge workflow that collects events from three parallel streams via FORK_JOIN, merges the results, and processes the merged output.
...
Each stream consumer is a simple, independent worker. a plain Java class that does one thing. Conductor takes care of collecting from all three streams in parallel via FORK_JOIN, merging the results after all complete, processing the merged output, retrying any failed stream independently, and tracking every merge operation.
