...
SQS Consumer .  receive an SQS message, validate it, process the event, and delete the message from the queue. Uses [Conductor](https://github.com/conductor-oss/conductor) to orchestrate independent services as workers ,  you write the business logic, Conductor handles retries, failure routing, durability, and observability for free.
...
Each SQS consumption concern is a simple, independent worker .  a plain Java class that does one thing. Conductor takes care of receiving the message, validating it, processing the event, and deleting it from the queue ,  retrying on transient failures, tracking every message's lifecycle, and resuming if the process crashes. You get all of that for free, without writing a single line of orchestration code.
...
Step 1: Registering task definitions...
  Registered: qs_receive_message, qs_validate_message, qs_process_message, qs_delete_message

Step 2: Registering workflow 'sqs_consumer_wf'...
  Workflow registered.
...
Step 4: Starting workflow...
  ...
  [qs_delete_message] Deleting message from queue:
  [qs_process_message] Processing message type:
  [qs_receive_message] Receiving message:
  [qs_validate_message] Validating message...

  Status: COMPLETED
  Output: {deleted=..., deletedAt=..., processed=..., result=...}
...
conductor workflow start \
  ...
  --version 1 \
  --input '{"queueUrl": "https://sqs.us-east-1.amazonaws.com/123456789/invoices-queue", "https://sqs.us-east-1.amazonaws.com/123456789/invoices-queue": "messageId", "messageId": "msg-fixed-001", "msg-fixed-001": "receiptHandle", "receiptHandle": "AQEBwJnKyrHigUMZj6rYigCgxlaS3SLy0a...", "AQEBwJnKyrHigUMZj6rYigCgxlaS3SLy0a...": "body", "body": "{\\", "{\\": "sample-{\\", "invoiceId\\": "INVOICE\\-001", "customerId\\": "CUST-1234", "amount\\": 249.99, "dueDate\\": "2025-01-15"}'
```
