...
class StopAcceptingWorkerTest {

    private final StopAcceptingWorker worker = new StopAcceptingWorker();

    @Test
    void taskDefName() {
        assertEquals("gs_stop_accepting", worker.getTaskDefName());
    }

    @Test
    void stopsAcceptingForValidInstance() {
        Task task = taskWith(Map.of("serviceName", "order-service", "instanceId", "pod-1"));
        TaskResult result = worker.execute(task);

        assertEquals(TaskResult.Status.COMPLETED, result.getStatus());
        assertEquals(true, result.getOutputData().get("stopped"));
    }
