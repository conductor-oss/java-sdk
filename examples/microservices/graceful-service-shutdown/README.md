...
Orchestrates graceful shutdown: stop accepting new work, drain in-flight tasks, checkpoint state, deregister from service registry, and terminate. Uses [Conductor](https://github.com/conductor-oss/conductor) to orchestrate independent services as workers .  you write the business logic, Conductor handles retries, failure routing, durability, and observability for free.
...
Step 1: Registering task definitions...
  Registered: gs_stop_accepting, gs_drain_tasks, gs_checkpoint, gs_deregister

Step 2: Registering workflow 'graceful_shutdown_workflow'...
  Workflow registered.
...
Step 4: Starting workflow...
  ...
  [checkpoint] State saved for
  [deregister]
  [drain] Waiting for 3 in-flight tasks to complete on
  [stop]

  Status: COMPLETED
  Output: {saved=..., instanceId=..., pendingTasks=..., deregistered=...}
...
conductor workflow start \
  ...
  --version 1 \
  --input '{"serviceName": "order-service", "order-service": "instanceId", "instanceId": "order-svc-pod-3", "order-svc-pod-3": "sample-order-svc-pod-3"}'
```
