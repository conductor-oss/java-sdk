...
public class ConductorClientHelper {
    ...
    public void registerTaskDefs(List<String> taskNames) {
        List<TaskDef> defs = taskNames.stream().map(name -> {
            TaskDef def = new TaskDef(); def.setName(name); def.setRetryCount(2);
            def.setTimeoutSeconds(60); def.setResponseTimeoutSeconds(30);
            def.setOwnerEmail("examples@orkes.io"); return def;
        }).toList();
    ...
    public String startWorkflow(String name, int version, Map<String, Object> input) {
        var request = new StartWorkflowRequest(); request.setName(name);
        request.setVersion(version); request.setInput(input);
