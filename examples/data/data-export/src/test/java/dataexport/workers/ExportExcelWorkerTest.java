...
class ExportExcelWorkerTest {

    private final ExportExcelWorker worker = new ExportExcelWorker();

    @Test
    void taskDefName() {
        assertEquals("dx_export_excel", worker.getTaskDefName());
    }
    ...
    void returnsFileInfo() {
        ...
        TaskResult result = worker.execute(task);
        assertEquals("export/data.xlsx", result.getOutputData().get("file"));
        assertEquals("Sheet1", result.getOutputData().get("sheetName"));
