...
echo "=== Orkes Cloud Connection (Java) ==="
echo ""

# Check for Orkes Cloud credentials first
if [[ -n "${CONDUCTOR_SERVER_URL:-}" && -n "${CONDUCTOR_KEY_ID:-}" && -n "${CONDUCTOR_KEY_SECRET:-}" ]]; then
    echo "Cloud credentials detected. Connecting to Orkes Cloud..."
    echo ""
...
echo "No cloud credentials. Using local Conductor..."
echo ""
...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    echo ""
    docker run --rm -v "$PWD":/work -v "$HOME/.m2":/root/.m2 -w /work maven:3.9-eclipse-temurin-21 mvn -q package -DskipTests 2>/dev/null || docker run --rm -v "$PWD":/work -v "$HOME/.m2":/root/.m2 -w /work maven:3.9-eclipse-temurin-21 mvn package -DskipTests
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" java -jar target/orkes-cloud-1.0.0.jar "$@"
else
