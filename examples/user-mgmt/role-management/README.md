...
A Java Conductor workflow example demonstrating Role Management. Uses [Conductor](https://github.com/conductor-oss/conductor) to orchestrate independent services as workers .  you write the business logic, Conductor handles retries, failure routing, durability, and observability for free.
...
```
=== Example 613: Role Management ===

Step 1: Registering task definitions...
  Registered: rom_request_role, rom_validate, rom_assign, rom_sync_permissions

Step 2: Registering workflow 'rom_role_management'...
  Workflow registered.
...
Step 4: Starting workflow...
  ...
  [assign] Role \"" + role + "\" assigned with
  [request] Role \"" + task.getInputData().get("role") + "\" requested for
  [sync] Permissions synced to IAM, API gateway, and database
  [validate] Role \"" + task.getInputData().get("role") + "\" approved for

  Status: COMPLETED
  Output: {assigned=..., permissions=..., requestId=..., logged=...}
...
conductor workflow start \
  ...
  --version 1 \
  --input '{"userId": "USR-ROM01", "USR-ROM01": "requestedRole", "requestedRole": "admin", "admin": "requestedBy", "requestedBy": "MGR-001", "MGR-001": "sample-MGR-001"}'
```
