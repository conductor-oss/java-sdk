...
echo "=== Twilio Integration (Java) ==="
echo ""

# Mode detection
if [ -n "$TWILIO_ACCOUNT_SID" ] && [ -n "$TWILIO_AUTH_TOKEN" ]; then
    echo "Mode: LIVE (Twilio credentials detected)"
else
    echo "Mode: SIMULATED (no Twilio credentials)"
    echo "  Set TWILIO_ACCOUNT_SID and TWILIO_AUTH_TOKEN for live SMS."
fi
echo ""
...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" java -jar target/twilio-integration-1.0.0.jar "$@"
else
