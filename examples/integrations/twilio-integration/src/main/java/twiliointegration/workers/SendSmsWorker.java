...
public class SendSmsWorker implements Worker {
    ...
    public TaskResult execute(Task task) {
        ...
        if (liveMode) {
            ...
            System.out.println("  [send] SMS " + message.getSid() + " to " + to + ": \"" + body + "\"");
        } else {
            String messageSid = "SM" + Long.toString(System.currentTimeMillis(), 36);
            System.out.println("  [send] SMS " + messageSid + " to " + to + ": \"" + body + "\"");
