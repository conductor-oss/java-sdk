...
echo "=== Hubspot Integration (Java) ==="
echo ""

if [ -n "$HUBSPOT_API_KEY" ]; then
    echo "Mode: LIVE (HUBSPOT_API_KEY is set)"
else
    echo "Mode: SIMULATED (set HUBSPOT_API_KEY for live mode)"
fi
echo ""
...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" java -jar target/hubspot-integration-1.0.0.jar "$@"
else
