...
echo "=== Azure Integration (Java) ==="
echo ""

if [ -n "$AZURE_CLIENT_ID" ]; then
    echo "Mode: LIVE (AZURE_CLIENT_ID is set)"
else
    echo "Mode: SIMULATED (set AZURE_CLIENT_ID, AZURE_CLIENT_SECRET, AZURE_TENANT_ID for live mode)"
fi
echo ""
...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" java -jar target/azure-integration-1.0.0.jar "$@"
else
