...
echo "=== Database Integration (Java) ==="
echo ""

if [ -n "$DB_URL" ]; then
    echo "Mode: LIVE (DB_URL is set)"
else
    echo "Mode: SIMULATED (set DB_URL for live mode)"
fi
echo ""
...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" java -jar target/database-integration-1.0.0.jar "$@"
else
