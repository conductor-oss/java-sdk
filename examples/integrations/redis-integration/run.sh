...
echo "=== Redis Integration (Java) ==="
echo ""

if [ -n "$REDIS_URL" ]; then
    echo "Mode: LIVE (REDIS_URL is set)"
else
    echo "Mode: SIMULATED (set REDIS_URL for live mode)"
fi
echo ""
...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" java -jar target/redis-integration-1.0.0.jar "$@"
else
