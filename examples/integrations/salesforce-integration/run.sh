...
echo "=== Salesforce Integration (Java) ==="
echo ""

if [ -n "$SF_CLIENT_ID" ]; then
    echo "Mode: LIVE (SF_CLIENT_ID is set)"
else
    echo "Mode: SIMULATED (set SF_CLIENT_ID, SF_CLIENT_SECRET, SF_USERNAME, SF_PASSWORD for live mode)"
fi
echo ""
...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" java -jar target/salesforce-integration-1.0.0.jar "$@"
else
