...
echo "=== Cloudwatch Integration (Java) ==="
echo ""

if [ -n "$AWS_ACCESS_KEY_ID" ]; then
    echo "Mode: LIVE (AWS_ACCESS_KEY_ID is set)"
else
    echo "Mode: SIMULATED (set AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY for live mode)"
fi
echo ""
...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" java -jar target/cloudwatch-integration-1.0.0.jar "$@"
else
