...
echo "=== Slack Integration (Java) ==="
echo ""

# Mode detection
if [ -n "$SLACK_BOT_TOKEN" ]; then
    echo "Mode: LIVE (Slack bot token detected)"
else
    echo "Mode: SIMULATED (no Slack bot token)"
    echo "  Set SLACK_BOT_TOKEN for live Slack message posting."
fi
echo ""
...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" java -jar target/slack-integration-1.0.0.jar "$@"
else
