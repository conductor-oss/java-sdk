...
echo "=== Gcp Integration (Java) ==="
echo ""

if [ -n "$GOOGLE_APPLICATION_CREDENTIALS" ]; then
    echo "Mode: LIVE (GOOGLE_APPLICATION_CREDENTIALS is set)"
else
    echo "Mode: SIMULATED (set GOOGLE_APPLICATION_CREDENTIALS for live mode)"
fi
echo ""
...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" java -jar target/gcp-integration-1.0.0.jar "$@"
else
