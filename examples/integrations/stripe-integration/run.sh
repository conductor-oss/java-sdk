...
echo "=== Stripe Integration (Java) ==="
echo ""

# Mode detection
if [ -n "$STRIPE_API_KEY" ]; then
    echo "Mode: LIVE (Stripe API key detected)"
else
    echo "Mode: SIMULATED (no Stripe API key)"
    echo "  Set STRIPE_API_KEY for live Stripe API calls."
fi
echo ""
...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" java -jar target/stripe-integration-1.0.0.jar "$@"
else
