...
echo "=== Teams Integration (Java) ==="
echo ""

if [ -n "$TEAMS_WEBHOOK_URL" ]; then
    echo "Mode: LIVE (TEAMS_WEBHOOK_URL is set)"
else
    echo "Mode: SIMULATED (set TEAMS_WEBHOOK_URL for live mode)"
fi
echo ""
...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" java -jar target/teams-integration-1.0.0.jar "$@"
else
