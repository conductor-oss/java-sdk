...
echo "=== Sendgrid Integration (Java) ==="
echo ""

# Mode detection
if [ -n "$SENDGRID_API_KEY" ]; then
    echo "Mode: LIVE (SendGrid API key detected)"
else
    echo "Mode: SIMULATED (no SendGrid API key)"
    echo "  Set SENDGRID_API_KEY for live email sending."
fi
echo ""
...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" java -jar target/sendgrid-integration-1.0.0.jar "$@"
else
