...
echo "=== Zendesk Integration (Java) ==="
echo ""

if [ -n "$ZENDESK_API_TOKEN" ] && [ -n "$ZENDESK_SUBDOMAIN" ]; then
    echo "Mode: LIVE (ZENDESK_SUBDOMAIN and ZENDESK_API_TOKEN are set)"
else
    echo "Mode: SIMULATED (set ZENDESK_SUBDOMAIN, ZENDESK_EMAIL, ZENDESK_API_TOKEN for live mode)"
fi
echo ""
...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" java -jar target/zendesk-integration-1.0.0.jar "$@"
else
