...
class ProcessBWorkerTest {

    private final ProcessBWorker worker = new ProcessBWorker();

    @Test
    void taskDefName() {
        assertEquals("sf_process_b", worker.getTaskDefName());
    }

    @Test
    void returnsLaneBWithItemCount() {
        Task task = taskWith(Map.of("items", List.of("x", "y", "z")));
        TaskResult result = worker.execute(task);

        assertEquals(TaskResult.Status.COMPLETED, result.getStatus());
        assertEquals("B", result.getOutputData().get("lane"));
