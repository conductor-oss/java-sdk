...
class NsCompleteWorkerTest {

    private final NsCompleteWorker worker = new NsCompleteWorker();

    @Test
    void taskDefName() {
        assertEquals("ns_complete", worker.getTaskDefName());
    }

    @Test
    void handlesCompletionStep() {
        Task task = taskWith(Map.of("region", "US", "tier", "premium", "amount", 500));
        TaskResult result = worker.execute(task);

        assertEquals(TaskResult.Status.COMPLETED, result.getStatus());
        assertEquals("ns_complete", result.getOutputData().get("handler"));
    ...
    void outputIsAlwaysDeterministic() {
        ...
        Task task2 = taskWith(Map.of("region", "EU", "tier", "standard", "amount", 200));
