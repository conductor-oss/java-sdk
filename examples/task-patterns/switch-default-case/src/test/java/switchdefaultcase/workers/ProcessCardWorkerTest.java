...
class ProcessCardWorkerTest {

    private final ProcessCardWorker worker = new ProcessCardWorker();

    @Test
    void taskDefName() {
        assertEquals("dc_process_card", worker.getTaskDefName());
    }

    @Test
    void returnsStripeHandler() {
        Task task = taskWith(Map.of("paymentMethod", "credit_card"));
        TaskResult result = worker.execute(task);

        assertEquals(TaskResult.Status.COMPLETED, result.getStatus());
        assertEquals("stripe", result.getOutputData().get("handler"));
    }

    @Test
    void outputIsAlwaysDeterministic() {
        Task task1 = taskWith(Map.of("paymentMethod", "credit_card"));
        Task task2 = taskWith(Map.of("paymentMethod", "credit_card"));
