...
Each tracking concern is an independent worker .  change detection, diff computation, classification, and recording. Conductor runs them in sequence: detect a change, compute the diff, classify it, then record it. Every change event is tracked with full context ,  you can see exactly what changed, when, and how it was classified. You get all of that for free, without writing a single line of orchestration code.
...
```
=== Example 427: Change Tracking ===

Step 1: Registering task definitions...
  Registered: chg_detect_change, chg_diff, chg_classify, chg_record

Step 2: Registering workflow 'change_tracking_427'...
  Workflow registered.
...
Step 4: Starting workflow...
  ...
  [classify]
  [detect] Detected change in
  [diff] Diffing
  [record] Recording

  Status: COMPLETED
  Output: {changeType=..., riskLevel=..., changeDetected=..., previousVersion=...}
...
conductor workflow start \
  ...
  --version 1 \
  --input '{"resourceType": "service", "service": "resourceId", "resourceId": "api-gateway", "api-gateway": "changeSource", "changeSource": "ci-cd-pipeline", "ci-cd-pipeline": "sample-ci-cd-pipeline"}'
```
