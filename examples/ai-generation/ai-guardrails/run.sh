...
echo "=== ai-guardrails Ai Guardrails (Java) ==="
echo ""
...
if [ -n "$CONDUCTOR_OPENAI_API_KEY" ]; then
    ...
    echo "Mode: SIMULATED (set CONDUCTOR_OPENAI_API_KEY for live AI guardrails)"
fi
echo ""
...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" CONDUCTOR_OPENAI_API_KEY="${CONDUCTOR_OPENAI_API_KEY:-}" java -jar target/ai-guardrails-1.0.0.jar "$@"
else
