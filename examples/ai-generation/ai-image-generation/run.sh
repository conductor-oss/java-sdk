...
echo "=== ai-image-generation Ai Image Generation (Java) ==="
echo ""
...
if [ -n "$CONDUCTOR_OPENAI_API_KEY" ]; then
    echo "Mode: LIVE (CONDUCTOR_OPENAI_API_KEY detected — GenerateWorker will call DALL-E 3)"
else
    echo "Mode: SIMULATED (set CONDUCTOR_OPENAI_API_KEY to generate real images with DALL-E 3)"
fi
echo ""
...
if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ...
    CONDUCTOR_SERVER_URL="$CONDUCTOR_SERVER_URL" java -jar target/ai-image-generation-1.0.0.jar "$@"
else
