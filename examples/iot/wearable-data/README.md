...
A Java Conductor workflow example that orchestrates a wearable health data pipeline .  collecting vital signs from wearable devices (heart rate, SpO2, step count, sleep data), processing raw readings into derived metrics (average heart rate, activity summaries), detecting anomalies in vital signs (irregular heartbeat, abnormal SpO2 drops), and notifying users or caregivers when health anomalies are found. Uses [Conductor](https://github.com/conductor-oss/conductor) to orchestrate independent services as workers ,  you write the business logic, Conductor handles retries, failure routing, durability, and observability for free.
...
```
=== Example 544: Wearable Data ===

Step 1: Registering task definitions...
  Registered: wer_collect_vitals, wer_process_data, wer_detect_anomalies, wer_notify

Step 2: Registering workflow 'wearable_data_demo'...
  Workflow registered.
...
Step 4: Starting workflow...
  ...
  [collect] Processing task
  [anomaly] Processing task
  [notify] Processing task
  [process] Processing task

  Status: COMPLETED
  Output: {done=..., anomalyCount=..., processed=..., avgHeartRate=...}
...
conductor workflow start \
  ...
  --version 1 \
  --input '{"userId": "USER-42", "USER-42": "deviceId", "deviceId": "WATCH-XR7", "WATCH-XR7": "sample-WATCH-XR7"}'
```
