...
class SearchWikiWorkerTest {

    private final SearchWikiWorker worker = new SearchWikiWorker();

    @Test
    void taskDefName() {
        assertEquals("sa_search_wiki", worker.getTaskDefName());
    }

    @Test
    void returnsResultsWithExpectedFields() {
        Task task = taskWith(Map.of(
                "queries", List.of("quantum computing"),
                "maxResults", 3));
        ...
        assertEquals("wikipedia", first.get("source"));
    }
    ...
    void returnsTotalScannedCount() {
        ...
        assertEquals(47, result.getOutputData().get("totalScanned"));
    }

    @Test
    void returnsSearchEngineWikipedia() {
        Task task = taskWith(Map.of(
                "queries", List.of("test"),
                "maxResults", 5));
        TaskResult result = worker.execute(task);

        assertEquals("wikipedia", result.getOutputData().get("searchEngine"));
    }
    ...
    @Test
    void allResultsHaveWikipediaSource() {
        Task task = taskWith(Map.of(
                "queries", List.of("test"),
                "maxResults", 5));
        TaskResult result = worker.execute(task);
        ...
        for (Map<String, Object> r : results) {
            assertEquals("wikipedia", r.get("source"));
        }
    }
