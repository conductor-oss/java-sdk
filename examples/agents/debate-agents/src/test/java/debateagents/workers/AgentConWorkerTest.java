...
class AgentConWorkerTest {

    private final AgentConWorker worker = new AgentConWorker();

    @Test
    void taskDefName() {
        assertEquals("da_agent_con", worker.getTaskDefName());
    }

    @Test
    void returnsConArgumentForRound1() {
        Task task = taskWith(Map.of("topic", "Microservices", "round", 1));
        TaskResult result = worker.execute(task);

        assertEquals(TaskResult.Status.COMPLETED, result.getStatus());
        assertEquals("CON", result.getOutputData().get("side"));
        assertEquals(1, result.getOutputData().get("round"));
        String argument = (String) result.getOutputData().get("argument");
        assertNotNull(argument);
        assertTrue(argument.contains("operational complexity"));
    }

    @Test
    void returnsConArgumentForRound2() {
        ...
        String argument = (String) result.getOutputData().get("argument");
        assertNotNull(argument);
        assertTrue(argument.contains("Network latency"));
    }

    @Test
    void returnsConArgumentForRound3() {
        ...
        String argument = (String) result.getOutputData().get("argument");
        assertNotNull(argument);
        assertTrue(argument.contains("organizational overhead"));
    }
    ...
    void parseRoundValues() {
        assertEquals(1, AgentConWorker.parseRound(null));
        assertEquals(1, AgentConWorker.parseRound("invalid"));
        assertEquals(3, AgentConWorker.parseRound(3));
        assertEquals(2, AgentConWorker.parseRound("2"));
        assertEquals(5, AgentConWorker.parseRound(5L));
    }
