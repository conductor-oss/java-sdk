...
class Solver1WorkerTest {

    private final Solver1Worker worker = new Solver1Worker();

    @Test
    void taskDefName() {
        assertEquals("comp_solver_1", worker.getTaskDefName());
    }
    ...
    void returnsCreativeSolution() {
        ...
        assertEquals("creative", solution.get("approach"));
        assertEquals("AI-Powered Adaptive System", solution.get("title"));
        assertEquals("$120K", solution.get("estimatedCost"));
        assertEquals("6 months", solution.get("timeline"));
        assertEquals(9, solution.get("innovationScore"));
        assertEquals("medium-high", solution.get("riskLevel"));
    }
