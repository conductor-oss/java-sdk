...
class BookHotelWorkerTest {

    private final BookHotelWorker worker = new BookHotelWorker();

    @Test
    void taskDefName() {
        assertEquals("sfj_book_hotel", worker.getTaskDefName());
    }

    @Test
    void returnsHotelBookingId() {
        Task task = taskWith(Map.of("tripId", "TRIP-42"));
        TaskResult result = worker.execute(task);

        assertEquals(TaskResult.Status.COMPLETED, result.getStatus());
        assertEquals("HTL-001", result.getOutputData().get("bookingId"));
        assertEquals("hotel", result.getOutputData().get("type"));
    ...
    void returnsConsistentBookingId() {
        ...
        assertEquals("HTL-001", result1.getOutputData().get("bookingId"));
        assertEquals("HTL-001", result2.getOutputData().get("bookingId"));
    }
