...
/**
 * Map worker 2: counts occurrences of the search term in each document
 * of its partition.
 *
 * Input: partition (list of text strings), searchTerm
 * Output: mapped (list of maps with "docIndex" and "count")
 */
public class MprMap2Worker implements Worker {
    ...
    public TaskResult execute(Task task) {
        ...
        List<Map<String, Object>> mapped = MapWorkerUtil.countInPartition(partObj, searchTerm, "map-2");

        System.out.println("  [map-2] Processed " + mapped.size() + " documents for term \"" + searchTerm + "\"");
