...
Each scoring concern is a simple, independent worker. a plain Java class that does one thing. Conductor takes care of executing them in order (collect data, calculate factors, compute score, classify), retrying if a credit bureau is slow, tracking every scoring decision with full factor breakdown for Fair Lending compliance, and resuming from the last step if the process crashes.
