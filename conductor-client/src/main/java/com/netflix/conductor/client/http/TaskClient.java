...
public class TaskClient {
     ...
     * (the {@code TaskRunner} has its own dispatcher).
     */
    ...
    public Optional<String> evaluateAndUploadLargePayload(Map<String, Object> taskOutputData, String taskType) {
        try (ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream()) {
            ...
            // Always publish the size event so the canonical
            // `task_result_size_bytes` gauge stays consistent with python-sdk
            // and go-sdk regardless of whether external-payload enforcement is
            // enabled. The external-storage upload path below remains gated.
     ...
     *             ({@code task_ack_error_total} / {@code task_ack_failed_total}).
     *             Calling this 2-argument overload leaves those counters at
     *             zero because the taskType label is unknown.
     */
    @Deprecated
