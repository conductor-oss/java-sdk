...
final class S3FileTransferAdapter implements FileTransferAdapter {

    private final SignedUrlHttpTransfer http;

    S3FileTransferAdapter(OkHttpClient client) {
        this.http = new SignedUrlHttpTransfer(client);
    }

    @Override
    public String storageType() { return "S3"; }
    ...
    public String uploadPart(
            ...
            throws IOException {
        String etag = http.uploadRangeReturningHeader(
                signedUrl, source, offset, length, Map.of(), "ETag");
        if (etag == null || etag.isBlank()) {
            throw new NonRetryableFileTransferException(
                    "S3 part upload response did not include an ETag");
        }
        return etag;
    }
}
