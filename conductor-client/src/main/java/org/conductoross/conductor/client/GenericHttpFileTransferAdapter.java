...
/** Conservative fallback for a server storage type newer than this SDK. */
final class GenericHttpFileTransferAdapter implements FileTransferAdapter {

    private final SignedUrlHttpTransfer http;

    GenericHttpFileTransferAdapter(OkHttpClient client) {
        this.http = new SignedUrlHttpTransfer(client);
    }

    @Override
    public String storageType() { return "HTTP"; }
    ...
    public String uploadPart(
        ...
        throw new NonRetryableFileTransferException(
                "Generic HTTP multipart upload is not supported");
    }
}
